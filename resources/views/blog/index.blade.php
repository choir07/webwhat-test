@extends('layouts.app')

@section('title', 'All Posts - The Powerful Posts')

@section('content')
    <div class="max-w-7xl mx-auto px-4 py-8">
        <h1 class="text-3xl font-bold mb-8">All Posts</h1>

        @if($posts->count())
            <div class="space-y-6">
                @foreach($posts as $post)
                    <div class="blog-card rounded-lg shadow p-6 border">
                        <h2 class="text-2xl font-bold tracking-tight mb-2">
                            <a href="/blog/{{ $post->slug }}">{{ $post->title }}</a>
                        </h2>
                        <p class="description-text">{{ Str::limit(strip_tags($post->content), 150) }}</p>
                        <div class="mt-3 text-sm post-meta">
                            {{ $post->published_at ? $post->published_at->format('M d, Y') : 'Draft' }}
                        </div>
                    </div>
                @endforeach
            </div>

            <div class="mt-6">
                {{ $posts->links() }}
            </div>
        @else
            <p class="post-meta">No posts found.</p>
        @endif
    </div>
@endsection